...
def render_html_report(result: ComparisonResult, comparison_id: str) -> str:
    """Render a safe interactive HTML report from a comparison result."""
    ...
    policy = load_mapping()
    excluded_attributes = [
        item["target"] for item in policy.get("attribute_mappings", [])
        if item.get("enabled", True) and item.get("compare") is False
    ]
    ...
    return template.render(
        ...
        excluded_attributes_json=_script_safe_json(excluded_attributes),
        result_json_urlencoded=result_json_urlencoded,
    )


def _script_safe_json(data) -> str:
    """Escape script-closing sequences before embedding JSON in HTML."""
    raw = json.dumps(data)
    return raw.replace("</", "<\\/")
